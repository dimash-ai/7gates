...
The middle gate of the **3-gate flow** (`harness/README-3gate.md`). Invoke as
...
slice) or reuse it (later slices), per the worktree contract `harness/checklists/worktree.md`. Run from
the pipeline root:
...
- Read the design `harness/design/$1-design.md` and task `harness/tasks/$1.md`, then run
...
- Keep the tree green: run the repo's local checks (e.g. `make -C $2/.worktrees/$1 verify` or its
  ...
  counts to `harness/runs/$1-build.txt`.
...
You are GPT Codex, the reviewer. Step: build. First run 'git -C $2/.worktrees/$1 --no-pager diff' and 'git -C $2/.worktrees/$1 status' to see the uncommitted changes, and read the build log harness/runs/$1-build.txt. Review that diff against the design doc harness/design/$1-design.md and task harness/tasks/$1.md. Apply the build lens: correctness, regressions, missing tests, unhandled edge cases, security, and any deviation from the plan/design or scope creep beyond the single slice. You are read-only and must NEVER edit any file. Score 0-10 per harness/checklists/scoring-rubric.md and output the verdict EXACTLY as the charter specifies (Reviewer: GPT Codex, Step: build). Status is APPROVED only if Score is 9.0 or higher."
```

Then:

1. Ensure `harness/reviews/$1/` exists, then save **only the verdict block** to `harness/reviews/$1/B-build-verdict-1.md`. Do NOT paste the raw CLI transcript (write that to `harness/runs/` if you want it). **Increment per slice and per fix:** `B-build-verdict-2.md`, `-3`, …
2. STOP.
