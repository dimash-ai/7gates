...
The final gate of the **3-gate flow** (`harness/README-3gate.md`): **prove the change is correct
...
`harness/checklists/worktree.md`). Every `git` and `codex` command below targets it, never `$2`.
...
You are GPT Codex, the doer for the VERIFY step of $1 in worktree $2/.worktrees/$1. Run 'git -C $2/.worktrees/$1 --no-pager diff $3...HEAD' (the base branch is $3 — if it was omitted, use main) and 'git -C $2/.worktrees/$1 status', then verify the whole change against task harness/tasks/$1.md and design harness/design/$1-design.md in ONE pass, not a checklist. (a) Scrutinize the entire change for the defects a per-slice gate misses: cross-cutting races, resource leaks, security (authz, injection, SSRF, secrets, rate-limiting), silent data corruption, swallowed failures, and any success criterion not actually met — cite file:line. (b) Back the risky paths with proof: add or strengthen tests under $2/.worktrees/$1 covering the edge and error paths (not just the happy path), and run the repo's verification (e.g. 'make -C $2/.worktrees/$1 verify' or its test command) until it is green. (c) PRINT your full verification report to stdout — what you scrutinized, what each test proves, and the pass/fail counts. Do NOT try to write it under harness/: your workspace-write sandbox is scoped to $2/.worktrees/$1, a worktree of a DIFFERENT git repo than the pipeline's harness/ tree, so a write there will fail. Touch ONLY test files and fixtures under $2/.worktrees/$1; if you find a real bug in production code, STOP and report it for a gate-B fix — do NOT patch production code here."
```

**After codex returns, save its output (Opus, in-session).** GPT printed its report rather than
writing it (its sandbox can't reach `harness/`), so persist it now: write codex's verification report to
`harness/reviews/$1/C-verify-report.md` and the raw transcript to `harness/runs/$1-verify.txt` — the Opus
